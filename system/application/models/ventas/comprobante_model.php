...
class Comprobante_model extends Model {
    ...
    public function getComprobantes($filter = NULL){
        ...
        if (isset($filter->plateNumber) && $filter->plateNumber != '') {
            $where .= ' and cpd.CPDEC_Observacion LIKE "%' . $filter->plateNumber.'%"';
        }
        ...
        if ( isset($filter->count) && $filter->count == true ){
        	...
        	$cols = "cp.CPC_FechaRegistro, DATE(cp.CPC_FechaRegistro) CPC_FechaR, cp.CPC_Fecha, cp.CPP_Codigo, cp.CPC_Serie, cp.CPC_Numero, cp.CPP_Codigo_canje, cp.CPC_GuiaRemCodigo, cp.CPC_DocuRefeCodigo, cp.CPC_NombreAuxiliar, cp.CLIP_Codigo, cp.CPC_TipoDocumento, cp.CPC_total, cp.CPC_FlagEstado, cp.OCOMP_Codigo,  cp.DUA_Codigo,
                       m.MONED_Simbolo,
...
//ANULAR DOCUMENTO
   ...
   public function getDuaCodigo($dua){
        $sql = "SELECT c.*, 
                (CASE pr.PROVC_TipoPersona  WHEN '1'
                    THEN e.EMPRC_RazonSocial
                    ELSE CONCAT(pe.PERSC_Nombre , ' ', pe.PERSC_ApellidoPaterno, ' ', pe.PERSC_ApellidoMaterno) end) nombre,
                (CASE pr.PROVC_TipoPersona WHEN 1 THEN e.EMPRC_Ruc ELSE pe.PERSC_NumeroDocIdentidad end) as documento
                FROM cji_comprobante c
                LEFT JOIN cji_proveedor pr ON pr.PROVP_Codigo = c.PROVP_Codigo
                LEFT JOIN cji_persona pe ON pe.PERSP_Codigo= pr.PERSP_Codigo AND pr.PROVC_TipoPersona ='0'
                LEFT JOIN cji_empresa e ON e.EMPRP_Codigo= pr.EMPRP_Codigo AND pr.PROVC_TipoPersona='1' 
                WHERE c.DUA_Codigo LIKE '$dua' AND c.CPC_FlagEstado = 1";
