...
class Comprobante extends Controller{
    ...
    # INICIO
        ...
        public function datatable_comprobantes($tipo_oper = '', $tipo_docu = ''){
            ...
            $filter->plateNumber   = trim($this->input->post('plateNumber'));
            ...
            if ($listado_comprobantes != NULL) {
                foreach ($listado_comprobantes as $indice => $valor) {
                    ...
                    if ($tipo_oper == 'C') {
                        $ConversorDeNumero = "";
                        $numeroSerieCanjeado = $valor->DUA_Codigo;;
                        $comprobantesRelacion = "";
                        $pdfCompromiso = "";
    ...
    public function comprobante_nueva($tipo_oper = '', $tipo_docu = '', $sucursal = ''){
        ...
        $voucherCode = 'RC'.date('m')."-";
        $count = 1;
        $voucherNumber = $this->ocompra_model->getLastestVoucher();

        $validateVoucherCode = preg_match('/RC(\d+)-/', $voucherNumber[0]->CPC_voucherCode, $match);
        $validateVoucherCode = $match[1];

        if(empty($voucherNumber[0]->CPC_voucherCode) || $validateVoucherCode != date('m')){
            $voucherCode .= strval($count);
        }else {
            $separateVoucherCode = explode("-",$voucherNumber[0]->CPC_voucherCode);
            $voucherCode = $separateVoucherCode[0];
            $separateVoucherCode = intval($separateVoucherCode[1]);
            $separateVoucherCode += $count;
            $separateVoucherCode = strval($separateVoucherCode);

            $voucherCode .= "-".$separateVoucherCode;
        }
    ...
    public function comprobante_insertar(){
        ...
        if($tipo_oper == 'C' & $tipo_docu == 'F'){
            // $filter->IMPOR_Nombre = $importacion;
            $filter->DUA_Codigo = $duacodigo;
            $filter->CPC_voucherCode = $this->input->post("voucherCode");
        }
    ...
    public function comprobante_editar($codigo, $tipo_oper = 'V', $tipo_docu = 'F'){
        ...
        $voucherCode = $datos_comprobante[0]->CPC_voucherCode;
        $data['voucherCode']=   $voucherCode;
        $duacodigo          = $datos_comprobante[0]->DUA_Codigo;
        ...
        //Nuevo producto
          ...
          $data["servicioAsociado"] = $this->comprobante_model->getComprobanteServicio($codigo);
    ...
    public function comprobante_modificar(){
        ...
        if(!empty($codService)){
            $checkService = $this->comprobante_model->getComprobanteServicio($codigo);
            if(empty($checkService)){
                $this->comprobante_model->insertarComprobanteServicio($codigo, $codService);
            }else{
                $this->comprobante_model->setComprobanteServicio($codigo, $codService);
            }
        }
    ...
    public function deleteComprobanteServicio(){
        $id = $this->input->post('id');
        $this->comprobante_model->deleteComprobanteServicio($id);
    }
   ...
   /**gcbq json verificacion de cantidad por producto y cantidad por serie este metodo lo usa json de inventario guiarem y otros***/
        ...
        switch (FORMATO_IMPRESION) {
            case 1: //Formato para ferresat
                if ($tipo_docu != 'TICKET')
                    ...
                    //$this->lib_props->comprobante_pdf_a5($codigo);
    ...
    public function ver_reporte_pdf_commpras($anio)
    {
        ...
        require_once(APPPATH.'libraries/class.backgroundpdf.php');
        $this->cezpdf = new Cezpdf('a4');
            ...
            foreach ($detalle_comprobante as $key => $value) {
                if ($value->flagBS=="B") {
                    ...
                    $cKardex->tipo_oper         = 2; # 1: SALIDA 2: INGRESO 
                     if (!empty(trim($comprobante->DUA_Codigo))) {
                        $cKardex->tipo_movimiento   = "INGRESO POR COMPRA IMPORT";
                    }else{
        ...
        if ($detalle_comprobante>0) {
            foreach ($detalle_comprobante as $key => $value) {
                if ($value->flagBS=="B") {
                    $cantidad = $value->CPDEC_Cantidad;
                    $producto = $value->PROD_Codigo;
                    $preciopr = $value->CPDEC_Pu_ConIgv;
                    ...
                    if ($comprobante->CPC_TipoOperacion=="C") {
                        $filterProd = new stdClass();
                        if ($comprobante->MONED_Codigo=="1") {
                            $filterProd->PROD_UltimoCosto       = $preciopr;
                        }else{
                            $filterProd->PROD_UltimoCostoD      = $preciopr;
                        }
                        $update = $this->producto_model->modificarRegistro($producto, $filterProd);
                    }
                    ############################
                    # REGISTRO DE KARDEX
                    ############################
