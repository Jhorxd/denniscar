...
;
	...
	public function datatable_control(){
		...
		$filter->placa = $this->input->post('buscar_placa');
		$filter->serie = $this->input->post('serie');
		$filter->numero = $this->input->post('numero');
		$filter->marca = $this->input->post('marca');
		$filter->modelo = $this->input->post('modelo');

		$listado_control = $this->controlservicio_model->getControl($filter);

		$lista = array();
		if (count($listado_control) > 0) {
			foreach ($listado_control as $indice => $valor) {
				$fecha = $valor->CONT_Fecha;
				$codigo = $valor->CONT_Codigo;
				$numero = $this->lib_props->getNumberFormat($codigo,6);
				$comprobante = ($valor->CPC_Serie != NULL) ? "<a style='color:green;' href='".base_url()."index.php/ventas/comprobante/comprobante_ver_pdf/$valor->CPP_Codigo/a4' data-fancybox data-type='iframe'>".$valor->CPC_Serie."-".$this->lib_props->getNumberFormat($valor->CPC_Numero,6)."</a>" : "";
				...
				$estadoComprobante = $valor->flagComprobante;

				if($estado==1){

					$enviarcorreo = "<button type='button' onclick='open_mail(\"$codigo\")' class='btn btn-default enviarcorreo'>
					<img src='".$this->url."/images/send.png' class='image-size-1b'>
					</button>";

					$enviarWS = "<button type='button' onclick='open_WS(\"$codigo\")' class='btn btn-default enviarcorreo'>
					<img src='".$this->url."/images/WS.png' class='image-size-1b'>
					</button>";
					...
					if($estadoComprobante == '1'){
						...
						$editar = "<button type='button' onclick='editar_control(\"$codigo\")' class='btn btn-default'>
							<img src='".$this->url."/images/modificar.png' class='image-size-1b'>
							</button>";
						$eliminar = "<button type='button' onclick='eliminar_control(\"$codigo\")' class='btn btn-default'>
							<img src='".$this->url."/images/documento-delete.png' class='image-size-1b'>
							</button>";
					}

					// $pdfImprimir = "<button href='".$this->url."index.php/ventas/controlservicio/control_ver_pdf_conmenbrete/$codigo/1' data-fancybox data-type='iframe' class='btn btn-default'>
					...
					$eliminar     = "";
					$editar       = "";
					$pdfImprimir  = "SERVICIO ANULADO";
					$enviarcorreo = "";
					$enviarWS = "";
				}

				$ver = "<button type='button' onclick='ver_control(\"$codigo\")' class='btn btn-default'>
		...
		$json = array(
			...
			"recordsTotal"    => count($this->controlservicio_model->getControl($filter)),
			"recordsFiltered" => intval( count($this->controlservicio_model->getControl($filter)) ),
			"data"            => $lista
		);
