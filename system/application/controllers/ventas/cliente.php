	...
	public function datatable_cliente(){
		...
		if ($clienteInfo != NULL) {
			foreach ($clienteInfo as $indice => $valor) {
				...
				$btn_sucursales = ($valor->CLIC_TipoPersona == 0) ? "" : "<button type='button' onclick='sucursales($valor->EMPRP_Codigo, \"$valor->numero - $valor->razon_social\")' class='btn2 btn-default' title='Sucursales'>
				<img src='".$this->url."/images/sucursal.png' class='image-size-1l'>
				</button>";

				$btn_contactos = ($valor->CLIC_TipoPersona == 0) ? "" : "<button type='button' onclick='modal_contactos(\"$valor->EMPRP_Codigo\", \"$valor->PERSP_Codigo\", \"$valor->numero - $valor->razon_social\")' class='btn2 btn-default' title='Contactos'>
				...
				$btn_documentos = "<button type='button' onclick='docs_emitidos($valor->CLIP_Codigo, \"$valor->numero\", \"$valor->razon_social\")' class='btn2 btn-default'>
                                <img src='".$this->url."/images/icono-documentos.png' class='image-size-1l'>
                            </button>";
                $btn_vehiculos = "<button type='button' onclick='modal_vehiculo(\"$valor->CLIP_Codigo\", \"$valor->numero - $valor->razon_social\")' class='btn btn-default' title='Bancos'>
				<img src='".$this->url."/images/icon-car.png' class='image-size-1b'>
				</button>";
	...
	public function autocomplete_ruc(){
		...
		if ($consulta != NULL) {
			...
			foreach ($consulta AS $cliente => $value) {
				...
				$vendedor = $value->CLIC_Vendedor;
				$digemin = $value->CLIC_Digemin;
				$result[] = array("value" => $ruc, "label" => "$value->CLIC_CodigoUsuario | $ruc - $nombre", "nombre" => $nombre, "codigo" => $value->CLIP_Codigo, "ruc" => $ruc, "TIPCLIP_Codigo" => $value->TIPCLIP_Codigo, "tipoPersona" => $tipoPersona, "codigoEmpresa" => $codigoEmpresa, "vendedor" => $vendedor, "contactos" =>  $contactos, "digemin" =>  $digemin, "tipoDocIdentidad" => $tipoDocIdentidad);
			}
		}
		echo json_encode($result);
	}
